            ...
            try:
                ...
                for market in markets_names:
                    try:
                        ...
                        if not exchange_prices.get('bid') is None or exchange_prices.get('ask') is None:  # if not
                            # bid/ask price is none
                            percentage_diff1 = (exchange_prices['ask'] - exchange_prices['bid']) / \
                                               ((exchange_prices['ask'] + exchange_prices['bid']) / 2) * 100
                            if not percentage_diff1 > 2 or percentage_diff1 < -2:  # if bid/ask diff
                                # not less/more than 2%
                                if exchange_prices.get('quoteVolume') > 5000:  # quote volume must be greater
                                    # than 10000 usdt
                                    prices.append(
                                    ...
                                    print(f'Low quote volume for {market} on {exchange}')
                            else:
                                print('Information: difference is huge b/w ask/bid price. Ignoring.')
                        else:
                            print(f'bid/ask not available for {market} on {exchange}')
