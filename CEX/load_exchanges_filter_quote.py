...
markets for this/these quote assets from those exchanges.
...
class loadFilterMarkets:
    ...
    def exchange_selection(self, islist=None):
        while True:
            try:
                ...
                exchanges_to_compare = 0, exchanges_names.clear()
    ...
    def load_market_for_selected_quote_asset(self):
        ...
        if len(unique_quote_assets) == 0:
            print('No common asset... redirecting to select another pair of exchanges')
            self.exchange_selection()

        asset_to_load_market = input(
            f"select only one asset to proceed(project is in Initial stage) (Total: {len(unique_quote_assets)} assets): ")

        if len(asset_to_load_market) == 0:
            print('No common asset... redirecting to select another pair of exchanges')
            self.exchange_selection()
        ...
        for asset_id, asset in enumerate(assets_copied):  # ex- asset = 'usdt', asset_id = 0
            for exchange_id, exchange in enumerate(exchanges_names):  # ex- exchange = 'bybit', exchange_id = 0
                for asset_var_id, asset_var in enumerate(
                        assets_names):  # ex- asset_var = 'bybit_usdt', asset_var_id = 0
                    if asset_var.startswith(str(exchange)) and asset_var.endswith(str(asset)):
                        if (asset_var in globals()) and (exchange in globals()):
                            try:
                                ...
                                print("{}-{} markets: {}".format(exchange, asset, len(globals()[asset_var])))
    ...
    def find_common_markets(self, iscalledagain=None):
        ...
        if len(assets_copied) == 0 or iscalledagain:
            unique_quote_assets.clear()
            assets_copied.clear()
            assets_names.clear()
            exchanges_to_compare = 0
            exchanges_names.clear()
            unique_quote_assets.clear()
            quote_assets_specific_2_exchange.clear()
            variable_name = ''
            assets_names_final.clear()

            self.load_market_for_selected_quote_asset()
        ...
        try:
                ...
                if set(filtered_markets_variables).issubset(set(globals().keys())):
                    ...
                    for common_market in range(len(filtered_markets_variables)):
                        ...
                        if common_market == 0:
                            ...
                            if common_market == len(filtered_markets_variables) - 1:
                                # final_common_market = globals()[variable_name]
                                print("{}-{} common markets".format(len(globals()[variable_name]), variable_name))
            ...
            print("Error: check find_common_market() - " + str(e))

        finally:
            return globals()[variable_name], exchanges_names, assets_copied
    ...
    def create_variables_for_selected_exchanges_assets(self, exchanges_names=None, unique_quote_assets=None,
        ...
        try:
            ...
            print("Error: while loading market - " + str(e))
